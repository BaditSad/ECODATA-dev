  ...
  return (
    <div className="px-4 py-3.5">
        ...
        {series.map((point) => {
          ...
          return (
            <div
              ...
              style={{ height: "100%" }}
              title={`${point.hour.toISOString().slice(0, 16).replace("T", " ")} UTC — ${
                point.accepted
              } accepted, ${point.rejected} rejected, ${formatBytes(point.bytes)}`}
            >
              {total > 0 ? (
                <div
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1.5 border-t border-[var(--edl-border)] pt-3">
        ...
        <span className={TYPE.meta}>{formatBytes(totals.bytes)} of audio ingested</span>
        <span className={TYPE.meta}>
          peak {formatNumber(peak)} attempts/hour
        </span>
      </div>
    </div>
  );
