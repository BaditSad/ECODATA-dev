    ...
    <div className="px-4 py-3.5">
          ...
          return (
            ...
            >
              ...
              <div
                ...
                title={fill(t.audio.histogramBar, {
                  range: label,
                  n: bin.detections,
                  confirmed: bin.confirmed,
                  rejected: bin.rejected,
                })}
                role="img"
                aria-label={fill(t.audio.histogramBar, {
                  range: label,
                  n: bin.detections,
                  confirmed: bin.confirmed,
                  rejected: bin.rejected,
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1.5 border-t border-[var(--edl-border)] pt-3">
        <span className={TYPE.meta}>
          <span style={{ color: TONE.positive.fg }}>■</span>{" "}
          {fill(t.audio.histogramAuto, { pct: formatPercent(aboveThreshold / total, 1) })}
        </span>
        <span className={TYPE.meta}>
          <span style={{ color: TONE.attention.fg }}>■</span> {t.audio.histogramReview}
        </span>
        <span className={TYPE.meta}>
          <span style={{ color: TONE.critical.fg }}>■</span>{" "}
          {fill(t.audio.histogramSuppressed, {
            pct: formatPercent(belowGuestFloor / total, 1),
          })}
        </span>
      </div>
    </div>
