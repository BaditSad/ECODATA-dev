...
import {
  ...
  rotateGuestCodes,
  rotateLobbyCode,
} from "@/modules/domains/actions/access-codes";
...
export function AccessCodeManager({
  ...
  now: number;
  /** Read-only holders of the Domains grant still need to read codes out. */
  canWrite: boolean;
}) {
  const t = useMessages();
  ...
  function codeState(code: TenantAccessCodeRow): {
    phase: "revoked" | "scheduled" | "expired" | "winding" | "live";
    label: string;
    tone: "positive" | "attention" | "neutral" | "critical";
  } {
    if (code.revoked_at) return { phase: "revoked", label: t.codes.revoked, tone: "critical" };

    const from = new Date(code.valid_from).getTime();
    const until = new Date(code.valid_until).getTime();

    if (now < from) return { phase: "scheduled", label: t.codes.scheduled, tone: "neutral" };
    if (now > until) return { phase: "expired", label: t.codes.expired, tone: "neutral" };
    ...
    if (until - now <= overlapWindowMs) {
      return { phase: "winding", label: t.codes.winding, tone: "attention" };
    }
    return { phase: "live", label: t.codes.live, tone: "positive" };
  }
  ...
  return (
    <>
      <div className="border-b border-[var(--edl-border)] px-4 py-3.5">
        <p className={TYPE.eyebrow}>{t.codes.lobby}</p>
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          <span className={TYPE.meta}>
            {fill(t.codes.lastRotated, { date: formatDate(lobbyRotatedAt, locale) })}
          </span>
        </div>
        ...
        >
          <input
            ...
            placeholder={t.codes.customLobby}
            maxLength={32}
            className="console-input h-8 max-w-[15rem] font-mono uppercase"
            aria-label={t.codes.customLobby}
          />
          <button
            type="button"
            disabled={pending}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--edl-border)] px-4 py-3">
        <div className="min-w-0">
          <p className={TYPE.eyebrow}>{t.codes.guestPins}</p>
          <p className={`mt-0.5 ${TYPE.meta}`}>
            {liveCodes.length === 0
              ? t.codes.noneLive
              : liveCodes.length === 1
                ? t.codes.oneLive
                : fill(t.codes.overlapLive, { n: liveCodes.length })}
          </p>
        </div>
        {canWrite ? (
          <button
      ...
      <Table
        head={[t.codes.cycle, t.codes.code, t.codes.state, t.codes.valid, ""]}
        empty={t.codes.empty}
      >
        {codes.map((code) => {
          const state = codeState(code);
          const active =
            !code.revoked_at && (state.phase === "live" || state.phase === "winding");
          ...
          return (
            <Row key={code.id}>
              ...
              <Cell align="right">
                {formatDateTime(code.valid_from, locale)}
                {rangeSep}
                {formatDateTime(code.valid_until, locale)}
              </Cell>
              <Cell>
                {active && canWrite ? (
